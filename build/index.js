...
function getTestFolderPath$2(filePath, rootPath) {
  return path$4.join(rootPath + PYTHAGORA_UNIT_DIR$1, path$4.dirname(filePath).replace(path$4.resolve(rootPath), ""), path$4.basename(filePath, path$4.extname(filePath)));
}
...
async function _createTests2(filePath, funcToTest) {
  try {
    ...
    for (const [i, funcData] of uniqueFoundFunctions.entries()) {
      ...
      const formattedData = await _classPrivateMethodGet$1(this, _reformatDataForPythagoraAPI$1, _reformatDataForPythagoraAPI2$1).call(this, funcData, filePath, getTestFolderPath$1(filePath, this.rootPath));
      const {
        tests,
        error
